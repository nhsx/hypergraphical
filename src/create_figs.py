...
def pagerank_scatter(suc_col, pred_col, dis_col, tab):
    ...
    ax.text(0.001, pred_col.max() + 0.04, "Predecessor", color="orange", fontsize=5)

    ax.fill(y, x, color="green", alpha=0.1)
    ax.text(
        suc_col.max() + 0.06, 0.01, "Successor", color="green", rotation=90, fontsize=5
    )

    ax.scatter(x=suc_col, y=pred_col, s=200, c=range(0, len(dis_col)), alpha=0.5)
